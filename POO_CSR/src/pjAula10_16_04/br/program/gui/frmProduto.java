...
public class frmProduto extends JFrame implements ActionListener{
	...
	private Produto instanciar(){
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Codigo de produto invalido", "Validação", JOptionPane.ERROR_MESSAGE);
			return null;
		}
		try {
			p.setComprimento(Float.parseFloat(txtComprimento.getText()));
		}catch(NumberFormatException e) {
			JOptionPane.showMessageDialog(null, "Comprimento invalido", "Validação", JOptionPane.ERROR_MESSAGE);
			return null;
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Largura invalida", "Validação", JOptionPane.ERROR_MESSAGE);
			return null;
		}
